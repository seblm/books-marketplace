...
public class MoniteurPdm extends javax.swing.JFrame {
    ...
    private void bouttonActualiserMouseClicked(java.awt.event.MouseEvent evt) {// GEN-FIRST:event_bouttonActualiserMouseClicked
        try {
            ...
            // nombreTentatives * attenteParTentative = nombre de milisecondes
            // avant que n'échoue l'écoute.
            int numeroTentative = 0;
            synchronized (this) {
                while (numeroTentative < 5 && socket.getInputStream().available() == 0) {
                    try {
                        wait(attenteParTentative);
            ...
            if (socket.getInputStream().available() != 0) {
                ...
                labelStatut.setText("Aucune réponse de la part de " + champHote.getText() + " depuis "
                        + (nombreTentatives * attenteParTentative) + "ms.");
            in.close();
            out.close();
            socket.close();
        } catch (java.net.UnknownHostException e) {
