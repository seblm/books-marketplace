...
public class Requetes extends bourse.reseau.Bd {
    ...
    public final synchronized void declarationPlaceDeMarche(String nom) {
        String hoteLocal = "";
        try {
            hoteLocal = InetAddress.getLocalHost().getHostAddress() + ":" + pdm.getPort();
    ...
    /**
     * Récupère depuis la base de données le prix d'achat d'un livre, sachant
     * son id. */
    ...
    public void inscrireAgent(Agent agent) {
        ...
        try {
            ...
            try {
            	...
            	if (verbose) System.err.println(ePrime);
            }
        }
    }
    ...
    /**
     * Récupère l'adresse d'une place de marché.
    ...
    /**
     * Récupère les adresses de toutes les places de marché déclarées dans la
     * base de données.
     * @return une LinkedList remplie de PDMPro.
     */
    public final synchronized LinkedList<PDMPro> getAdressesPdm() {
        LinkedList<PDMPro> liste = new LinkedList<PDMPro>();
        try {
