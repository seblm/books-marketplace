...
public class ThreadLecture extends Thread {
    ...
    public void run() {
        ...
        try {
            while (managerConnexion.getEcouter()) {
                ...
                if (buffer == null) {
                    ...
                    if (message.lastIndexOf(managerConnexion.getMotifFin()) != -1) {
                        // Le message contient le motif de fin donc on peux demander son traitement.
                        if (managerConnexion.getVerbose()) System.out.println("Réception de : " + message);
            ...
            // La connexion a été coupée par le correspondant
            if (managerConnexion.getVerbose()) System.err.println("ThreadLecture : La connexion a été interrompue !!!");
            managerConnexion.setConnexionInterrompue(true);
            managerConnexion.traiter(null);
        }
        try { in.close(); } catch (IOException e) { if (managerConnexion.getVerbose()) System.err.println(e); }
    }
}
