...
public class ListeAgentMemoire {
    ...
    public void refresh() {
        ...
        javax.swing.table.DefaultTableModel tm = new javax.swing.table.DefaultTableModel(
            new String [] {"Nom", "Catégorie", "Présent", "Fréquence des catégories"},
            liste.size()
        );
    ...
    public void setPresent(boolean present) {
        Iterator parcours = this.liste.values().iterator();
        while (parcours.hasNext()) { ((AgentMemoire)parcours.next()).setPresent(present); }
    }
    /** Ajouter un agent concurrent. */
    public void ajouter(AgentMemoire a) { this.liste.put(a.getNom(), a); }
    /** Vrai si la liste contient un agent nommé nom. */
    public boolean contient(String nom) { return this.liste.containsKey(nom); }
    /** Met a jour la catégorie des adversaires et la fréquence des catégories
     *  de livres de chaque agent. */
    public void miseAJourCategorie() {
        Iterator parcours = this.liste.values().iterator();
        bourse.agent.sdd.AgentMemoire courant;
        /** Le nombre de livres de la catégorie i */
        int[] nb = new int[5];
        for (int i = 0; i<5; i++) nb[i] = 0;
        int total = 0;
        int maxnb;
        int ind;
        /** pour chaque agent, */
        while (parcours.hasNext()) {
            ...
            while (parcoursLivre.hasNext()) {
                categorieCourante = new bourse.protocole.Categorie(((Livre)parcoursLivre.next()).getCategorie()).getCode();
            ...
            /** si le total des transaction n'est pas nul, on met à jour les statistiques. */
            if (total != 0) {
                for (int i = 0; i<5; i++) courant.getStatsCategorie()[i] = (float)((float)nb[i]/(float)total);
            }
            /** mise à jour de la catégorie : la catégorie la plus fréquemment achetée. */
            maxnb = 0;
            ind = 5;
            for (int i = 0; i<5; i++) {
                if (maxnb < nb[i]) {
    ...
    /** Méthode de mise à jour globale à partir d'une liste d'agents.
    ...
    /** Mettre à jour les listes avec une liste de String représentant les noms des agents. */
    ...
    public String toString(int decalage) {
        String output = "";
        Iterator parcours = this.liste.values().iterator();
        while (parcours.hasNext()) { output += ((AgentMemoire)parcours.next()).toString(decalage) + "\n"; }
    ...
    public static void main(String[] argc) {
        ...
        Agent a1 = new bourse.agent.sdd.Agent("Arnaud"); // arnaud toujours present.
        Agent a2 = new bourse.agent.sdd.Agent("Sebastien"); // sebastien nouveau.
        Agent a3 = new bourse.agent.sdd.Agent("Jerôme"); // jerome de nouveau present.
        ListeAgent l = new ListeAgent();
        l.ajouter(a1); l.ajouter(a2); l.ajouter(a3);
